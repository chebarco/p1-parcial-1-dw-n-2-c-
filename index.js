...
function cargarPista() {
  ...
  do {
    nombrePista = prompt("Que tema te gusta? 🤔");
    if (nombrePista == "") {
      alert("No ingresaste ningun dato🥴, volve a intentar🙏");
    }
  } while (!(nombrePista !== ""));
...
function mostrarDisco() {
  if (discos.length > 0) {
    for (let disco of discos) {
      for (let duracionPista of pistas) {
        // color.textContent = duracionPista.getDuracion;
        ...
        html.innerHTML = `
            ...
            <h2 class="titulo-nombre">Pista: <p class="info-disco"  >${duracionPista.getnombrePista}</p></h2>
            <h2 class="titulo-nombre">Duracion:<p class="info-disco">${duracionPista.getDuracion}seg</p></h2>
            </div>
          </div> 
          `;
        
        // if (duracionPista.getDuracion > 180) {
